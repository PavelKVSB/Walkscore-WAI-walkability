...
# Walk distances 
walk_distances = [400,800,1200,2414] 

walk_speed = 1.34112

Beta_coeff = 180

# Amenity weights definition
amenity_weights = {
    "grocery": [3],
...
if locality=="Hradec":   
    ...
    ctverce_grid = ctverce_grid_.set_index('ID500M')
    amenities_layer_all = gpd.read_file("./DATA/Amenities/cile_ov_classic.shp")
else:
...
num_pois = max(num_pois_weights) # 10 pro normal walkscore
...
for walk_distance in walk_distances:
    ...
    final_score_3['ws_{}'.format(walk_distance)]= final_score_3.apply(apply_road_length, axis=1) 
    
    final_score_3_sub = final_score_3[['ws_{}'.format(walk_distance)]]
    ...
    final_score_3.to_csv("./DATA/Walkscore_results/classic_results/Walkscore_{}_{}m.csv".format(locality,walk_distance)) 
...
ctverce_grid_final.to_file("./DATA/Walkscore_results/classic_results/Walkscore_{}.shp".format(locality)) 