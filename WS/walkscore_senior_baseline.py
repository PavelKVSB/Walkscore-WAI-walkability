...
locality = "Hradec" # options: Ostrava, Hradec

walk_distance = 2414.02

walk_speeds = [0.4,0.5,0.6,0.7,0.8,0.9,1,1.1,1.2,1.3] 

Beta_coeffs = [140,144,149,153,158,162,167,171,176,180] 

amenity_weights = {
    "retail": [6.36363,4.24242],
    "hypermarket": [0.90909,0.60606],
    "entertainment": [0.60606,0.60606],
    "doctor": [1.51515],
    "park": [0.15151]
}
...
for walk_speed, Beta_coeff in zip(walk_speeds, Beta_coeffs):
    ...
    walk_speed_dot = str(walk_speed).replace(".", "")
    final_score_3['ws_{}_{}'.format(walk_speed_dot,Beta_coeff)]= final_score_3.apply(apply_road_length, axis=1) 

    final_score_3_sub = final_score_3[['ws_{}_{}'.format(walk_speed_dot,Beta_coeff)]]
    df_walkscore_temp.append(final_score_3_sub)

    print("Exporting partial results..")

    final_score_3.to_csv("./DATA/Walkscore_results/senior_results/baseline/Walkscore_{}_senior_{}_{}.csv".format(locality,walk_speed_dot,Beta_coeff)) 
...
ctverce_grid_final.to_file("./DATA/Walkscore_results/senior_results/baseline/Walkscore_{}_senior_baseline.shp".format(locality)) 